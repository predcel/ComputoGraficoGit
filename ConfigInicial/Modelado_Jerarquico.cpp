...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//modelos que controlar la posicion 
		//para no tener que inicializar el modelo
		...
		//genera matriz auxiliar, que es un pivote para que se vea logico el movimiento
		modelTemp = model = glm::translate(model, glm::vec3(1.5f, 0.0f, 0.0f)); 
		model = glm::scale(model, glm::vec3(3.0f, 1.0f, 1.0f));
		color = glm::vec3(0.0f, 1.0f, 0.0f); //generamos color para cada caja, modelo RGB
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);//A --- dibuja la caja
		...
		//dedo 1 Falange A
		model = glm::translate(modelTemp, glm::vec3(0.25f, 0.35f, 0.33f));
		...
		model = glm::rotate(model, glm::radians(dedo1FB), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		//dedo 2 Falange B
		model = glm::translate(modelTemp, glm::vec3(0.3f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(dedo2FB), glm::vec3(0.0f, 0.0f, 1.0f));
